...
export default class SequenceViewer extends React.Component {
    ...
    componentDidMount() {
        ...
        this.fv.onFeatureSelected(function (d) {
            ...
            selectFunc(chain, d.detail.start + that.pdbOffsets[chain],
    ...
    loadOffsets(res) {
        ...
        res.PDB.SIFTS.UniProt[unpID].mappings.forEach(chain => {
            this.unpOffsets[chain.chain_id] = chain.unp_start + this.seqResOffsets[chain.chain_id]
                - chain.start.residue_number - this.chainStartResN[chain.chain_id] + 1
        });

        this.pdbOffsets = {}
        res.PDB.SIFTS.UniProt[unpID].mappings.forEach(chain => {
            this.pdbOffsets[chain.chain_id] = -chain.unp_start + this.seqResOffsets[chain.chain_id]
                - this.chainStartResN[chain.chain_id] + 2
        });
    }
    ...
    loadFeatures(res) {
        ...
        res.PDB.SIFTS.UniProt[unpID].mappings.forEach(chain => {
            ...
            this.fv.addFeature({
                data: [{
                    x: chain.unp_start + this.seqResOffsets[chain.chain_id] - chain.start.residue_number + 1,
                    y: chain.unp_end + this.seqResOffsets[chain.chain_id] - chain.start.residue_number + 1,
                    description: name
                }],
                name: name,
                className: "test1",
            ...
            let markResidues = function (that, residues, title) {
                that.fv.addFeature({
                    data: residues.filter(r => r.Chain == chain.chain_id).map(r => {
                        return { x: that.pdbToUnp(r.Chain, r.Position), y: that.pdbToUnp(r.Chain, r.Position), description: name }
                    }),
