...
export default class StructureViewer extends React.Component {
    ...
    load(res) {
        ...
        let action = Transform.build()
            .add(this.state.plugin.context.tree.root, Transformer.Data.Download, { url: `/api/structure/cif/${id}`, type: 'String', id })
    ...
    clear() {
        LiteMol.Bootstrap.Command.Tree.RemoveNode.dispatch(this.state.plugin.context, this.state.plugin.context.tree.root);
    }
    ...
    highlight(chain, start, end) {
        ...
        let query = LiteMol.Core.Structure.Query.sequence('1', chain, { seqNumber: start }, { seqNumber: end });
    ...
    highlightResidues(residues) {
        ...
        var model = plugin.context.select('model')[0];
        residues.forEach(r => {
            let query = LiteMol.Core.Structure.Query.sequence('1', r.chain, { seqNumber: r.pos }, { seqNumber: r.pos });
            LiteMol.Bootstrap.Command.Molecule.Highlight.dispatch(plugin.context, { model, query, isOn: true });
        })
    };
    ...
    clearHighlight() {
        ...
        Object.keys(this.state.res.PDB.SeqResOffsets).forEach(chain => {
            let query = LiteMol.Core.Structure.Query.sequence('1', chain, { seqNumber: 1 }, { seqNumber: this.state.res.PDB.TotalLength });
    ...
    applyTheme() {
        ...
        plugin.command(LiteMol.Bootstrap.Command.Layout.SetState, { collapsedControlsLayout: LiteMol.Bootstrap.Components.CollapsedControlsLayout.Landscape, hideControls: true });
    };
